...
**الوضع الحالي: النظام مفتوح بلا شاشة دخول** (`open_access = true`).
الـ proxy ينشئ جلسة المالك تلقائياً، فيفتح الموقع مباشرةً.

⚠️ هذا يعني أن أي شخص يعرف الرابط يدخل بصلاحية المالك: يرى التكلفة
والأرباح وأرصدة الموزعين، ويستطيع التعديل والحذف.

المصادقة نفسها لم تُزَل ولا يصح إزالتها: كل سياسات RLS وفحوص الصلاحيات
وسجل التدقيق مبنية على `auth.uid()`، فبدون جلسة لا يعمل شيء في النظام.
ما تغيّر هو ظهور شاشة الدخول فقط.

للعودة إلى الدخول بالرمز:

```sql
update public.app_settings set value = 'false'::jsonb where key = 'open_access';
```
...
السطور المعلَّمة ✏️ في أعلاه (البريد والرمز والاسم)، والصق الملف كاملاً
في Supabase → SQL Editor → Run.

الملف مولَّد من الترحيلات ويبني القاعدة كاملة وينشئ حساب المالك برمز
الدخول في خطوة واحدة. وهو معاملة واحدة: إن فشل أي سطر لا يُطبَّق شيء،
فتصحّح وتعيد التشغيل بأمان. يُعاد توليده بـ `npm run bootstrap:build`،
ويتحقق CI من مطابقته للترحيلات فلا يتحول إلى وثيقة كاذبة.
